            ...
            <div class="form">
              ...
              <?php
                ...
                $detail = $doctor->schedules
                  ? $doctor->schedules->where('appointment_time', $schedule)->first()
                  : collect([]);

                if ($detail) {
                  $start = explode('-', $detail->appointment_time)[0];
                  $end = explode('-', $detail->appointment_time)[1];
                } else {
                  $start = explode('-', $schedule)[0];
                  $end = explode('-', $schedule)[1];
                }
              ...
              <div class="slot">
                  <div class="form-group">
                    <label class="col-md-3 control-label" for="status0">Status</label>
                    <div class="col-md-6">
                          <div class="btn-group pull-left btn_status_list">
                            ...
                            <input type="hidden" name="status" value="{{ $detail && $detail->status ? $detail->status : "Available" }}">
                          </div>
                    </div>
                  </div>
                  <div class="form-group">
                      ...
                      <div class="col-md-6">
                      	<div class="input-group bootstrap-timepicker"><input type="text" class="timepicker-default form-control start" placeholder="eg. 9.00 AM" value="{{ $start }}" /><span class="input-group-addon">to</span><input type="text" class="timepicker-default form-control end" placeholder="eg. 1.00 PM" value="{{ $end }}"/></div>
                      </div>
                  </div>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-md-6">
                        <input type="number" name="max_patients" class="form-control maxp" placeholder="eg. 25" style="width: 100px" min="0" value="{{ $detail && $detail->max_patients ? $detail->max_patients : "" }}">
                      </div>
                  </div>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-md-9">
                        ...
                        <div class="col-md-4 text-left byDaysPerMonth-{{ $index }}">
                          <label>
                            <input name="bulk_{{ $index }}" type="radio" value="0" required {{ $detail && $detail->bulk_option == 0 ? "checked" : "" }} />
                            By Days / Month
                          </label>
                          <div class="clearfix"></div>
                          <span class="inline">Every
                          <select multiple="multiple" name="bulkValues_1_{{ $index }}[]" style="height: 180px" class="form-control daybymonth" required>
                          ...
                          of
                          <select multiple="multiple" name="bulkValues_2_{{ $index }}[]" style="height: 200px;" class="form-control bymonth" required>
                        ...
                        <div class="col-md-4 text-left byDaysPerYear-{{ $index }}">
                          ...
                          <select multiple="multiple" name="bulkValues_1_{{ $index }}[]" style="height: 180px;" class="form-control daybyyear" required>
                            <option>- Select -</option>
                              <option value="Monday"
                          ...
                          of
                          <select multiple="multiple" name="bulkValues_2_{{ $index }}[]" style="height: 200px;" class="form-control byyear" required>
...
<script>
  ...
  function initStatus(element) {
    var currentStatus = element.find("input[name=status]").val();

    changeStatus(currentStatus, element);
  }

  function changeStatus(selectedStatus, element) {
    var statusList = ['Available', 'Not Available', 'On Clinic', 'Doctor on Leave', 'Appointment Full'];
    ...
    element.find("input[name=status]").val(selectedStatus);
  ...
  function getClassName(status) {
    ...
    if (status == 'On Clinic') return 'btn-info';
