									...
									?>
										...
										<?php for($d = 1; $d <= $days_in_month; $d++,$day++ ):?>
											<?php
												...
												$dayStatus = \App\Schedule::where('doctor_id',$consultant->id)->where('date',$appointmentdate)->first();
												...
												if(isset($consultant->schedules[0])&& $consultant->schedules[0]->bulk_option == 0){
													if($consultant->schedules[0]->bulk_values != '' && in_array($month_name, explode(',',explode(';' , $consultant->schedules[0]->bulk_values)[1])) && $consultant->schedules[0]->status != "Available") {

													}
												}
												if ($consultant->schedules[0]->bulk_values != '') {
...
//                                        		$booked_str = $dayStatus !== null ? $dayStatus->status : "Available";
                                        ...
                                        if (!empty($scheduleData)) {
											...
											@if ($diff > 0)
												...
												@else
												<td width="14%" class="passdate"><div class="margin-bottom-10"></div><span class="date-numer" id="{{$d}}" data-toggle="tooltip" data-placement="top" title="{{$consultant->schedules[0]->status}}">{{$d}}</span><br/> <div class="margin-bottom-25"></div><span class="text-danger">{{$booked_str}}</span></td>
											@endif
											@else
										...
										<div class="col-md-12">
											<h4>Please select a time slot:
												...
												<input type="radio" data-index="{{ $index }}" name="schedule_id"
															 ...
															 {{ $index == 0 ? 'checked required' : '' }} /> <div style="margin-left: 35px; margin-top: -20px; line-height: 1.42">{{ str_replace("-", " - " , str_replace(": ", "", $schedule)) }}</div>
												@endforeach
											</h4>
