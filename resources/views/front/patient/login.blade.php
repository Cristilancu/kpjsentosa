...
@section('content')
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<hr>
							<form method="post" action="{{url("/patient/reset_password/")}}">
							<div class="form-group">
								<label class="col-md-4">Email (Login ID): <span class="text-danger">*</span></label>
								<div class="col-md-8">
									<input type="email" name="email" class="form-control" placeholder="eg. mail@yourdomain.com" required="required">
								</div>
							</div>
	...
	<h2 class="light bordered main-title">Free <span>Signup</span></h2>
	...
	<form class="form-horizontal" method="post" action="/patient/register">
		...
		<p>Sign up with us to get the latest promotions & packages from KPJ Sentosa KL Specialist Hospital and book or update your appointments online. Please fill up the following registration form.</p>
		...
		<div class="col-md-6">
			...
			<div class="height10"></div>
				...
				<div class="form-group">
					...
					<div class="col-md-8">
						<input type="date" placeholder="eg. 25 June, 1980" class="form-control" name="date_of_birth" value="{{old('date_of_birth')}}"> 
					</div>
				</div>
				<div class="clearfix"></div>
		...
		<div class="form-group">
			...
			<div class="col-md-8">
				<select class="form-control" name="country_id">
					...
					<option value="{{$country->id}}" data-calling-code="{{$country->calling_code}}" data-eu-tax="{{$country->eu_tax}}" {{(old('country_id')==$country->id?'selected="selected"':'')}} {{('Malaysia'==$country->name?'selected="selected"':'')}}>{{$country->name}}</option>
					@endforeach
				</select>
			</div>
		</div>
